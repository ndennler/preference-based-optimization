...
import numpy as np
import pandas as pd
from utils import load_archive_as_np, calculate_reward_alignment, calculate_regret, calculate_query_qualities
import sys
...
for DIM_EMBEDDING in [4, 8, 16, 32]:
    ...
    for name, generator in zip(['Random', 'CMA-ES', 'CMA-ES-IG'], [random_generator, cma_es, cma_es_ig]):
        ...
        for trial in tqdm(range(NUM_TRIALS)):
            user_estimate.reset()
            true_preference = np.random.uniform(low=-1, high=1, size=DIM_EMBEDDING)
            dataset_measures, dataset_objectives = load_archive_as_np(DIM_EMBEDDING, np.random.randint(5))
