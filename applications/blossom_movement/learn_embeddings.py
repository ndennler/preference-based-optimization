...
class Conv1dAutoencoder(nn.Module):
    def __init__(self, latent_size=32):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(in_channels=4, out_channels=16, kernel_size=12, stride=2, padding=5),
            nn.ReLU(True),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.LeakyReLU(True),
            nn.ConvTranspose1d(in_channels=16, out_channels=4, kernel_size=12, stride=2, padding=5, output_padding=1),
        )
...
embeddings = np.empty((data.shape[0], 32))  # Assuming latent_size=32
...
np.save('blossom_unnormalized_reconstructions.npy', unnormalized_reconstructions)
np.save('blossom_embeddings.npy', embeddings)